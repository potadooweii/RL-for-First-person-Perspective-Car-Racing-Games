...
from models.TD3_model import ActorNet, CriticNet
...
class CarRacingTD3Agent(TD3BaseAgent):
    def __init__(self, config):
        ...
        self.env = CarRacingEnvironment(
            self.scenario, discrete_action=False, test=False
        )
        self.test_env = CarRacingEnvironment(
            self.scenario, discrete_action=False, test=True
        )
        ...
        self.actor_net = ActorNet(
            channel_dim, state_dim, action_dim, self.backbone, self.motor_magnitude
        )
        self.critic_net1 = CriticNet(channel_dim, state_dim, action_dim, self.backbone)
        self.critic_net2 = CriticNet(channel_dim, state_dim, action_dim, self.backbone)
        ...
        self.target_critic_net1 = CriticNet(
            channel_dim, state_dim, action_dim, self.backbone
        )
        ...
        self.max_action = torch.tensor([self.motor_magnitude, 1], device=self.device)
        self.min_action = torch.tensor([0.01, -1], device=self.device)
    ...
    def decide_agent_actions(self, state, sigma=0):
        ...
        state = (
            torch.from_numpy(state).to(self.device, dtype=torch.float32).unsqueeze(0)
        )
        ...
        if sigma != 0:
            ...
            exploration_noise = torch.tensor(
                self.noise.generate(), device=self.device
            ).clamp(
                self.noise_clip_rate * self.min_action,
                self.noise_clip_rate * self.max_action,
            )
            action = (action + exploration_noise).clamp(
    ...
    def update_behavior_network(self):
        ...
        with torch.no_grad():
            ...
            noise = torch.tensor(
                self.noise.generate(), dtype=torch.float32, device=self.device
            ).clamp(
                self.noise_clip_rate * self.min_action,
                self.noise_clip_rate * self.max_action,
            )
            next_action = (self.target_actor_net(next_state) + noise).clamp(
                self.min_action, self.max_action
            )
