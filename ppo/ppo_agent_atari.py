	...
	def update(self):
		...
		for _ in range(self.update_count):
			for start in range(0, sample_count, self.batch_size):
				...
				_, action_logprob, value, entropy = self.net(ob_train_batch, old_action=ac_train_batch.squeeze())
				logp_pi_train_batch = logp_pi_train_batch.squeeze()

				# calculate policy loss
				ratio = torch.exp(action_logprob - logp_pi_train_batch)
				clipped_ratio = torch.clamp(ratio, 1 - self.clip_epsilon, 1 + self.clip_epsilon)
				surrogate_loss = (-torch.min(ratio * adv_train_batch, clipped_ratio * adv_train_batch)).mean()
				...
				loss = surrogate_loss - self.entropy_coefficient * entropy_loss + self.value_coefficient * v_loss
