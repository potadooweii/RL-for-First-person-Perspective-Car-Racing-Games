...
from rainbow_DQN.sum_tree import SumTree
...
class N_Steps_Prioritized_ReplayBuffer(object):
    def __init__(self, args):
        ...
        self.n_steps = args.n_steps
        self.n_steps_deque = deque(maxlen=self.n_steps)
        self.buffer = {'state': np.array([np.zeros(args.state_dim) for _ in range(self.buffer_capacity)]),
                       'action': np.zeros((self.buffer_capacity, 1)),
                       'reward': np.zeros(self.buffer_capacity),
                       'next_state': np.array([np.zeros(args.state_dim) for _ in range(self.buffer_capacity)]),
                       'terminal': np.zeros(self.buffer_capacity),
                       }
        self.current_size = 0
        self.count = 0
    ...
    def sample(self, total_steps, device):
        ...
        for key in self.buffer.keys():  # numpy->tensor
            if key == 'action':
                batch[key] = torch.tensor(self.buffer[key][batch_index], dtype=torch.long, device=device)
            else:
                batch[key] = torch.tensor(self.buffer[key][batch_index], dtype=torch.float32, device=device)

        return batch, batch_index, torch.tensor(np.asarray(IS_weight), dtype=torch.float, device=device)
