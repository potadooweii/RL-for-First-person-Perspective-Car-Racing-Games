...
class PPOBaseAgent(ABC):
    ...
    def train(self):
        ...
        while self.total_time_step <= self.training_steps:
            observation, info = (
                self.env.reset(test=True) if episode_idx % 5 == 0 else self.env.reset()
            )
            episode_reward = 0
            episode_len = 0
            episode_idx += 1
            ...
            while True:
                ...
                if terminate or truncate:
                    ...
                    print(
                        f"[{len(self.gae_replay_buffer)}/{self.update_sample_count}][{self.total_time_step}/{self.training_steps}]  episode: {episode_idx}  episode reward: {episode_reward}  episode len: {episode_len}"
                    )
                    break
            ...
            lrnow = 0.9 * frac * self.lr + 0.1 * self.lr
